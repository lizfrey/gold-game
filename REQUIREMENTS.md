...
1. Start from the commandline of the form above; thus the first argument is the pathname for a [map file](#mapfiles) and the second argument is an optional seed for the random-number generator; if provided, the seed must be a positive integer.
...
7. Display a status line on the first line of the display, as noted in the [protocol](#networkprotocol) below.
...
If the player's `real name` is empty, the server shall respond with
